            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white p-6">
                <div class="flex justify-between items-center mb-6">
                    <div class="flex items-center">
                        <button style="margin-right: 3cm" onclick="window.location.href='pacientes/registrar_pacientes'" type="button" class="text-gray-600 hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-green-300 rounded-lg text-sm px-2.5 py-2.5 bg-transparent">
                            <i class="fas fa-user-plus"></i>
                        </button>
                        <form method="GET" action="{{ route('paciente') }}" class="flex">
                            <input type="text" name="search" placeholder="Buscar paciente" class="bg-gray-100 text-sm px-4 py-2.5 rounded-md outline-blue-500" value="{{ request('search') }}">
                            ...
                            @foreach ($pacientes as $paciente)
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 flex space-x-2">
                                        ...
                                        <div class="rounded-md overflow-hidden">
                                            <a href="{{ route('paciente.show', $paciente->id) }}" class="block text-center text-blue-600 hover:text-blue-900 px-3 py-2">Ver Detalles</a>
                                        </div>
                                        <div class="rounded-md overflow-hidden">
                                            <form action="{{ route('paciente.destroy', $paciente->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este paciente?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="block text-center text-red-600 hover:text-red-900 px-3 py-2">Eliminar</button>
                                            </form>
                                        </div>
                                        <div class="rounded-md overflow-hidden">
